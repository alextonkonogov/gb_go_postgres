...
func GetAuthorById(ctx context.Context, dbpool *pgxpool.Pool, id int) (a author, err error) {
	row := dbpool.QueryRow(ctx, `select id, name, surname from authors where id = $1`, id)
	if err != nil {
		err = fmt.Errorf("failed to query data: %w", err)
		return
	}

	err = row.Scan(&a.Id, &a.Name, &a.Surname)
	if err != nil {
		err = fmt.Errorf("failed to query data: %w", err)
		return
	}
